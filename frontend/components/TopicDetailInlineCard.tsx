...
interface TopicDetailInlineCardProps {
  guide: TopicGuide;
  onClose: () => void;
}

export default function TopicDetailInlineCard({
  guide,
  onClose,
}: TopicDetailInlineCardProps) {
  return (
    <div className="rounded-[28px] border-2 border-[#2c73b9]/30 bg-white p-6 sm:p-8 shadow-[0_12px_40px_rgba(44,115,185,0.08)] space-y-8 relative my-4 animate-fadeIn">
      <button
        onClick={onClose}
        className="absolute right-5 top-5 flex items-center gap-1.5 rounded-full bg-[#f4f8fc] px-3 py-1.5 text-xs font-bold text-[#5a6e7f] border border-[#d2dfec] hover:bg-[#9a2020] hover:text-white hover:border-[#9a2020] transition-all shadow-xs"
        title="Tutup Panduan Detail"
      >
        <X size={14} />
        <span className="hidden sm:inline">Tutup Panduan</span>
      </button>

      <div className="space-y-3 border-b border-[#edf3f9] pb-6 pr-14">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="inline-flex items-center gap-1.5 rounded-full border border-[#d2dfec] bg-[#f4f8fc] px-3 py-1 text-xs font-semibold text-[#5a6e7f]">
            Detail Panduan & Resep Stunting
          </span>
        </div>
        <h2 className="text-2xl sm:text-3xl font-extrabold text-[#0f2d4a]">
          {guide.title}
        </h2>
        <p className="text-sm text-[#5a6e7f] leading-relaxed max-w-3xl">
          {guide.subtitle}
        </p>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {guide.dailyNeeds.map((need, idx) => (
            <div
              key={idx}
              className="rounded-2xl border border-[#d2dfec] bg-[#f9fcff] p-5 shadow-xs space-y-3"
            >
        ...
        {guide.fluidsAndSupplements.length > 0 && (
          <div className="rounded-xl border border-[#d2dfec] bg-[#f4f8fc] p-4 flex flex-wrap items-center justify-around gap-3 text-xs font-semibold text-[#0f2d4a]">
      ...
      <div className="rounded-2xl border border-[#d2dfec] bg-[#f4f8fc] p-5 sm:p-6 space-y-2">
